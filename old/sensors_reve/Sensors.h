...
public:
 	ADC(int bus = 1, uint16_t address = 0x48, uint8_t gain = 5, uint8_t speed = 7, uint8_t channel = 0, double gainFactor = .000125, uint8_t conversionMode = 0) {
		//uint8_t test = 2;
		//cout << "Channel " << (unsigned int)channel << (unsigned int)test << endl;
		i2c = new I2CDevice(bus, address);
		mode = conversionMode;
		config[0] = 0x01;
		config[1] = (channel << 4) | (gain << 1) | 0; //currently it always runs continuous, but the mode allows reset of config
		config[2] = speed << 5;
		i2c->send(config, 3);
		factor = gainFactor; //edit to allow all gains
	}
	~ADC() {
		delete i2c;
	}
	...
	//If mode is in continuous, then you can only read one set ie one differential
	//Set mode to 1 to read a single
	int getRaw() {

		if (mode == 1) {
			i2c->send(config, 3);
		}
		usleep(4000);//need to wait? yes
